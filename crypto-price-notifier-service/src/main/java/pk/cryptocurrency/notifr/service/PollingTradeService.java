...
@Profile("polling")
@Deprecated
public class PollingTradeService implements TradeService {
    private MarketDataService marketDataService;

    public PollingTradeService(MarketDataService marketDataService) {
    ...
    private class TradePollingGenerator implements Consumer<FluxSink<Trade>> {
        ...
        private long fetchTrades(Long fromTimestamp, SynchronousSink<List<Trade>> sink) {
            try {
                ...
                List<Trade> tradesList = trades.getTrades().stream()
                        .map(PollingTradeService.this::convertTrade)
